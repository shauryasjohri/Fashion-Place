...
function Product({ link, imgSrc, price, name, onAddToCart, isInCart }) {
  return (
    <Card
      imgSrc={imgSrc}
    ...
    >
      <div className={clsx(
        ...
        {isInCart ? (
          <Link to="/cart" aria-label={`${name} is in cart, go to cart`}>
            <ProductButton className="!bg-green-500 text-white">
              <Check className='min-w-8' />
            </ProductButton>
          </Link>
        ) : (
          <ProductButton onClick={onAddToCart} aria-label={`Add ${name || 'product'} to cart`}>
            <ShoppingCart className='min-w-8' />
          </ProductButton>
        )}
        <Link to={link} aria-label={`View ${name || 'product'} details`}>
          <ProductButton>
